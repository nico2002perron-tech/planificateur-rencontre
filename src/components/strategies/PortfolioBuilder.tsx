...
import {
  ...
  GripVertical, ChevronDown, PieChart, Loader2, Upload, DollarSign,
} from 'lucide-react';
import { ImportModal, type ImportedHolding } from './ImportModal';
...
interface Props {
  ...
  initialHoldings?: Array<{ symbol: string; name: string; weight: number }>;
  initialName?: string;
...
export function PortfolioBuilder({
  ...
  const [portfolioName, setPortfolioName] = useState(initialName ?? 'Portefeuille Modèle');
  const [benchmark, setBenchmark] = useState(initialBenchmark ?? 'XIU.TO');
  const [portfolioValue, setPortfolioValue] = useState<string>(
    initialPortfolioValue ? initialPortfolioValue.toString() : ''
  );
  ...
  // Import holdings from modal
  const handleImport = useCallback((imported: ImportedHolding[]) => {
    const newHoldings: Holding[] = imported
      .filter(h => !holdings.some(ex => ex.symbol === h.symbol))
      .map(h => ({
        id: crypto.randomUUID(),
        symbol: h.symbol,
        name: h.name,
    ...
    setHoldings(prev => {
      const combined = [...prev, ...newHoldings];
      // Re-equalize weights when importing
      const w = Math.round(100 / combined.length * 10) / 10;
      return combined.map(h => ({ ...h, weight: w }));
    });
  }, [holdings]);
  ...
  const runAnalysis = useCallback(async () => {
    ...
    const pv = parseFloat(portfolioValue) || undefined;
  ...
  }, [holdings, benchmark, portfolioName, portfolioValue, onAnalysisComplete]);

  const totalWeight = holdings.reduce((s, h) => s + h.weight, 0);
  const isValid = holdings.length >= 2 && totalWeight > 0;
  const weightOk = Math.abs(totalWeight - 100) < 1;
  const pvNum = parseFloat(portfolioValue) || 0;
    ...
    <>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {/* Portfolio value + Import button */}
          <div className="flex items-center gap-3">
            <div
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white flex-1 max-w-xs transition-all focus-within:ring-2 focus-within:ring-[#1CB0F6]/30"
              style={{ border: '2px solid #e5e7eb', borderBottom: '3px solid #d1d5db' }}
            >
              ...
              <input
                ...
                onBlur={() => {
                  ...
                  if (n > 0) setPortfolioValue(Math.round(n).toString());
                }}
                className="flex-1 text-sm font-bold text-text-main placeholder:text-text-muted/60 focus:outline-none bg-transparent"
                placeholder="Valeur du portefeuille (optionnel)"
              />
              {pvNum > 0 && (
                <span className="text-xs text-text-muted flex-shrink-0 whitespace-nowrap">
                  {pvNum >= 1_000_000
                    ? `${(pvNum / 1_000_000).toFixed(1)}M`
                    : pvNum >= 1_000
                      ? `${(pvNum / 1_000).toFixed(0)}K`
                      : ''
                  }
                </span>
              )}
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-xs font-bold text-text-muted hover:text-[#1CB0F6] transition-all"
              style={{ border: '2px solid #e5e7eb', borderBottom: '3px solid #d1d5db' }}
            >
          ...
          {holdings.length === 0 ? (
            <div
              className="rounded-2xl p-10 text-center cursor-pointer hover:border-[#1CB0F6]/30 transition-colors"
              style={{ border: '2px dashed #e5e7eb' }}
              onClick={() => setShowImport(true)}
            >
              <Upload className="h-10 w-10 text-text-muted/30 mx-auto mb-3" />
              <p className="text-sm font-bold text-text-muted">Aucune position</p>
              <p className="text-xs text-text-muted/70 mt-1">
                Recherchez des titres ci-dessus ou <span className="text-[#1CB0F6] font-bold">importez un relevé</span> (Excel, CSV, photo).
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 px-4 py-2">
                ...
                {pvNum > 0 && (
                  <span className="w-20 text-[10px] font-bold text-text-muted uppercase tracking-wider text-right">Valeur</span>
                )}
                <span className="w-8" />
              </div>
              ...
              {holdings.map((h, idx) => (
                ...
                >
                  ...
                  {pvNum > 0 && (
                    <span className="w-20 text-right text-xs font-bold text-text-muted">
                      {((pvNum * h.weight / totalWeight) || 0) >= 1000
                        ? `${((pvNum * h.weight / totalWeight) / 1000).toFixed(1)}K`
                        : `$${Math.round(pvNum * h.weight / totalWeight)}`
                      }
                    </span>
                  )}
                  <button
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <button
                  onClick={() => setHoldings([])}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold text-text-muted hover:text-red-500 hover:bg-red-50 transition-all"
                >
                ...
                <div className={`text-xs font-bold px-3 py-1.5 rounded-lg ${
                  ...
                  {pvNum > 0 && ` · $${pvNum.toLocaleString('fr-CA')}`}
                </div>
              </div>
            </div>
          )}
